...
class FlowNet2(FlowNetBase):
    ...
    def graph_structure(self, x):
        ...
        flownetc_flow = resize(flownetc_flow2 * 20., mode='bilinear')
        ...
        flownets1_flow = resize(flownets1_flow2 * 20., mode='bilinear')

        resampled_img1 = resample(x2, flownets1_flow)
        norm_diff_img0 = channel_norm(x1 - resampled_img1)

        # FlowNet-S
        concat2 = tf.concat([x1x2, resampled_img1, flownets1_flow / 20., norm_diff_img0], axis=1)
        with tf.variable_scope('flownet_s2'):
            flownets2_flow2 = FlowNet2S().graph_structure(concat2, preprocess=False)

        flownets2_flow = resize(flownets2_flow2 * 20., mode='nearest')
